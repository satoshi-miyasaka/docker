...
cursor.execute("SELECT ball_1, ball_2, ball_3, ball_4, ball_5, ball_6, ball_7, bonus1, bonus2 FROM loto7")
...
list = []
dict = {}
for ball in range(1, 38):
	list.append(ball)
	dict[ball] = 0

# 新しい順に取得
# cursor.execute("SELECT * FROM loto6 order by number desc")
cursor.execute("SELECT ball_1, ball_2, ball_3, ball_4, ball_5, ball_6, ball_7, bonus1, bonus2 FROM loto7 order by number desc")
rows = cursor.fetchall()
